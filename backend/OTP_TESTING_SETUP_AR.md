...
- عند تفعيل `OTP_APP_BYPASS=1` سيقبل النظام أي 4 أرقام لأي رقم جوال
- يجب أولًا استدعاء `/api/accounts/otp/send/` حتى توجد OTP pending record
- لا تحتاج `OTP_APP_BYPASS_ALLOWLIST` مع السلوك الحالي

### تفعيل bypass في الإنتاج أو بيئة التطوير

إذا كنت تحتاج تشغيل الدخول مؤقتًا بدون مزود SMS:

| Key | Value |
|-----|-------|
| `OTP_APP_BYPASS` | `1` |

- بعد التفعيل، أي رقم جوال سيقبل أي 4 أرقام بعد استدعاء `/api/accounts/otp/send/`
