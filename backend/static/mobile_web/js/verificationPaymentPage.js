/* ===================================================================
   verificationPaymentPage.js — Dedicated verification checkout page
   =================================================================== */
'use strict';
...
const VerificationPaymentPage = (() => {
  ...
  function verificationUrl() {
    const node = shell();
    return (node && node.dataset && node.dataset.verificationUrl) || '/verification/';
  }

  function detailUrlTemplate() {
    const node = shell();
    return (node && node.dataset && node.dataset.detailUrlTemplate) || '/api/verification/requests/__id__/';
  }

  function listUrl() {
    const node = shell();
    return (node && node.dataset && node.dataset.listUrl) || '/api/verification/requests/my/';
  }
  ...
  async function fetchRequestById(requestId) {
    ...
    if (!response.ok || !response.data) {
      throw new Error(apiErrorMessage(response, 'تعذر تحميل تفاصيل طلب التوثيق.'));
    }
  ...
  async function findRequestByInvoice(invoiceId) {
    ...
    if (!response.ok || !response.data) {
      throw new Error(apiErrorMessage(response, 'تعذر تحميل طلبات التوثيق.'));
    }
  ...
  function hydrateFromRequest(requestItem) {
    ...
    const invoiceSummary = requestItem.invoice_summary || {};
    const invoiceId = parseInt(String(requestItem.invoice || invoiceSummary.id || ''), 10);
    ...
    state.requestCode = safeText(requestItem.code) || (state.requestId ? ('AD' + String(state.requestId).padStart(6, '0')) : '—');
    state.invoiceCode = safeText(invoiceSummary.code) || ('IV' + String(invoiceId).padStart(6, '0'));
    state.requestStatus = safeText(requestItem.status).toLowerCase();
    ...
    const invoicePaid = state.invoiceStatus === 'paid';
    const requestNeedsPayment = state.requestStatus === 'pending_payment';
    state.canPay = requestNeedsPayment && !invoicePaid;

    if (invoicePaid) {
      setStatusBanner('تم سداد هذه الفاتورة مسبقًا. يمكنك العودة لصفحة التوثيق.', 'success');
  ...
  async function loadRequestData() {
    ...
    try {
      ...
      if (!requestItem) {
        throw new Error('تعذر العثور على طلب التوثيق المرتبط بهذا الرابط.');
      }
  ...
  async function submitPayment() {
    ...
    const idempotencyKey = 'verify-checkout-' + String(state.requestId || '0') + '-' + String(state.invoiceId);
    ...
    try {
      ...
      state.requestStatus = 'active';
      renderState();
      setStatusBanner('تم سداد الفاتورة بنجاح وحفظ حالة الدفع. يمكنك متابعة حالة الطلب من صفحة التوثيق.', 'success');

      const nextUrl = verificationUrl() + '?payment=success&request_id=' + encodeURIComponent(String(state.requestId || ''));
      window.setTimeout(() => {
