...
def seed_app_intro_preview_block(apps, schema_editor):
    SiteContentBlock = apps.get_model("content", "SiteContentBlock")

    preview_block, created = SiteContentBlock.objects.get_or_create(
        key="app_intro_preview",
        defaults={
            "title_ar": "بروفة التعريف بالتطبيق",
            "body_ar": "",
            "is_active": True,
        },
    )
    ...
    if created:
        source_block = SiteContentBlock.objects.filter(key="onboarding_intro").first()
        if source_block and getattr(source_block, "media_file", None):
            preview_block.media_file = source_block.media_file.name
            preview_block.save(update_fields=["media_file", "updated_at"])
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.AlterField(
            ...
            field=models.CharField(
                choices=[
                    ...
                    ("app_intro_preview", "بروفة التعريف بالتطبيق"),
        ...
        migrations.RunPython(seed_app_intro_preview_block, noop_reverse),
    ]