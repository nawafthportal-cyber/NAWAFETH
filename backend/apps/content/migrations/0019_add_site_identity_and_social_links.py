...
def seed_site_identity_blocks(apps, schema_editor):
    ...
    defaults = {
        ...
        "topbar_brand_subtitle": {
            "title_ar": "المنصة الرقمية للخدمات",
            "body_ar": "",
            "is_active": True,
        },
        "footer_brand_title": {
        ...
        "footer_brand_description": {
            "title_ar": "وصف المنصة في الفوتر",
            "body_ar": "منصة تجمعك بالمختصين والعروض والخدمات في تجربة أوضح وأسرع على الويب.",
            "is_active": True,
        },
        "footer_copyright": {
            "title_ar": "جميع الحقوق محفوظة لمنصة نوافــذ",
            "body_ar": "",
            "is_active": True,
        },
    }
...
def collapse_site_links_to_singleton(apps, schema_editor):
    SiteLinks = apps.get_model("content", "SiteLinks")
    rows = list(SiteLinks.objects.order_by("-updated_at", "-id"))
    if not rows:
        return

    latest = rows[0]
    singleton, _ = SiteLinks.objects.get_or_create(pk=1)
    fields = [
        "x_url",
        "instagram_url",
        "snapchat_url",
    ...
    for field in fields:
        setattr(singleton, field, getattr(latest, field, ""))
    singleton.save()
    SiteLinks.objects.exclude(pk=1).delete()
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("content", "0018_add_branding_content_keys"),
    ]
    ...
    operations = [
        ...
        migrations.AlterField(
            ...
            field=models.CharField(
                choices=[
                    ...
                    ("topbar_brand_title", "الشريط العلوي - اسم المنصة"),
                    ("topbar_brand_subtitle", "الشريط العلوي - وصف المنصة"),
                    ("footer_brand_title", "الفوتر - اسم المنصة"),
                    ("footer_brand_description", "الفوتر - وصف المنصة"),
                    ("footer_copyright", "الفوتر - حقوق النشر"),
        ...
        migrations.RunPython(seed_site_identity_blocks, noop_reverse),
        migrations.RunPython(collapse_site_links_to_singleton, noop_reverse),
    ]
