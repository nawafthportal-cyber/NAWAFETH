...
from apps.providers.models import ProviderLike


class Command(BaseCommand):
    help = "Clean duplicate ProviderLike rows, keeping the oldest row per (user, provider)."
    ...
    def handle(self, *args, **options):
        ...
        duplicate_groups = list(
            ProviderLike.objects.values("user_id", "provider_id")
        ...
        if not duplicate_groups:
            self.stdout.write(self.style.SUCCESS("No duplicate ProviderLike rows found."))
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Deleted {deleted_total} duplicate ProviderLike row(s) across {len(duplicate_groups)} group(s)."
            )
        )
