...
if SECRET_KEY == "dev-secret-key-change-me":
    warnings.warn(
        "DJANGO_SECRET_KEY is not set – using insecure default. "
        "Set DJANGO_SECRET_KEY in Render environment variables for production security.",
        stacklevel=1,
    )
...
FEATURE_RBAC_ENFORCE = env_bool("FEATURE_RBAC_ENFORCE", True)
RBAC_AUDIT_ONLY = env_bool("RBAC_AUDIT_ONLY", False)
...
# Keep backend deterministic so collectstatic always produces staticfiles.json.
...
# Emergency-only fallback for broken deploys. Keep disabled by default.
ENABLE_STATICFILES_FALLBACK = env_bool("ENABLE_STATICFILES_FALLBACK", False)
if ENABLE_STATICFILES_FALLBACK:
	STATICFILES_BACKEND = _fallback_static_backend
	WHITENOISE_USE_FINDERS = True
	print(
		f"[settings] WARN: ENABLE_STATICFILES_FALLBACK=1; using {_fallback_static_backend}."
	)
else:
